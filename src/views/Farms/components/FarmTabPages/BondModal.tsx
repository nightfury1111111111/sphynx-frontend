...
const BondWrapper = styled.p<{ currentTab: string }>`
  ...
  cursor: pointer;
  padding: 5px;
  border-bottom: ${({ currentTab }) => (currentTab === 'Bond' ? 'solid #C42BB4' : 'none')};
  color: ${({ currentTab }) => (currentTab === 'Bond' ? '#C42BB4 ' : '#ffffff ')};
`

const RedeemWrapper = styled.p<{ currentTab: string }>`
  margin: 20px;
  cursor: pointer;
  color: ${({ currentTab }) => (currentTab === 'Redeem' ? '#C42BB4 ' : '#ffffff')};
  padding: 5px;
<<<<<<< HEAD
  border-bottom: ${({ currentTab }) => (currentTab === 'Redeem' ? 'solid #C42BB4' : 'none')};;
=======
  border-bottom: ${({ currentTab }) => (currentTab === 'Redeem' ? 'solid #C42BB4' : 'none')}; ;
>>>>>>> edeff3d6ee71e2fc9aa54514c08a5e2372926a4c
`
    ...
    <StyledModalBackground onClick={handleCloseModal}>
      ...
      >
        ...
        <StyledModalBody>
          ...
          {currentTab === 'Bond' ? (
            <>
              <DivWrapper>{account ? null : <ConnectWalletButton />}</DivWrapper>
              <Sperate />
